@if(isset($SeoKeywords) && isset($city_id) && isset($course_id))

@foreach ($SeoKeywords as $keyword)

@if($keyword->course == $course_id && $keyword->city == $city_id)
@php
...
@endphp
@extends('layouts.header',[$ktitlez?? null,$meta?? null, $ogtitle??null, $ogtype??null, $ogurl??null, $ogdesc??null, $canonicals??null])
@break
@endif
@endforeach
@endif
...
#msearchresponsive{
    ...
    <div class="clh">
        <div class="clh__c">
            ...
            @endif
            @if (isset($cityname))
                <p class="clh__c-t">{{ $coursename }} Exam in {{ $cityname }}, List of top {{ $coursename }} Coaching
                    Institutes in {{ $cityname }}</p>
                        ...
                        <div class="form">
                            <form>
                                ...
                                <div class="button-container">
                                    ...
                                    <a class="button button-secondary" href="{{ url('mapsearch') . '/' . $typename . '/' . $courseslug . '/' . $cityname }}" id="msearch">MapSearch</a>
                                </div>
              ...
              <a class="button button-secondary" href="{{ url('mapsearch') . '/' . $typename . '/' . $courseslug . '/' . $cityname }}"id="msearchresponsive">MapSearch</a>       
                   @else
                @if (isset($course))
                    <div class="course">
                        <h2>{{ $course->name }}</h2>
        ...
        Array.of($('.type-btn-dropdown').children()).forEach((city) => {
            city.on("click", (e) => {
                ...
                if (typeSelected) {
                    ...
                    $('#mapview').attr('href',
                        `{{ url('mapsearch') . '/' }}${typeSelected}/${courseSelected}/${citySelected}`
                        );
                } else {
                    $('#mapview').attr('href', `{{ url('mapsearch') . '/' }}${e.target.innerText}`);
                }
    ...
    <div class="bc">
        <div class="bc__c">
            ...
            <p>&nbsp; > {{ isset($typename) ? $typename : '' }}{{ isset($coursename) ? ' > ' . $coursename : '' }}
                {{ isset($cityname) ? ' > ' . $cityname : '' }}</p>
        </div>
    </div>
    ...
    <div class="cl">
        <div class="cl__c">
            <div class="cl__c-f">
                <div class="cl__c-f-c">
                    ...
                    @if(isset($citylocals))
                    @foreach ($citylocals as $citylocal)
                    <div class="locality_filter" style="margin-left:10px;font-size:1.4rem;font-family:nunito;margin-bottom:10px;">
                        <label>
                            <input type="checkbox" class="filteritem localities_checkbox" name="selected_citylocals" value="{{ $citylocal->id }}">
                            {{ $citylocal->name }}
                        </label>
                    </div>
                    @endforeach
        ...
        @if(isset($SeoKeywords) && isset($city_id) && isset($course_id))
               ...
               @if($SeoKeyword->course == $course_id && $SeoKeyword->city == $city_id)


                <p style="width:100%;margin-bottom:10px;padding:6px 8px;background-color:white;font-size:15px;font-family:nunito;text-align:center" ><b>{{$SeoKeyword->key1 }} <a href="{{ url('/')}}" class="keyword" id="keyhover">{{$SeoKeyword->key2}}</a> {{$SeoKeyword->key3}}</b></p>
                    @endif
                    @endforeach
                    @endif
                ...
                <div class="cl__c-cl-c">
                    ...
                    @else
                        @foreach ($coachings as $coaching)
                            @include('components.coachingcard', [
                                'coaching' => $coaching,
                        ...
                        @endforeach
                        @if (!isset($actionType))
                            @include('components.pagination', ['data' => $coachings])
                        @endif
                    @endif
                </div>
            </div>
...
@if(isset($pccs))
    <div class="popular" style="margin-left:5%;background-color:#fff;margin-right:5%;margin-bottom:10px;">
        <h2 style="padding:4px 6px;font-family:nunito;font-size:20px">{{$coursename?? null}} Coachings in Popular Cities</h2>
        ...
        @foreach ($pccs as $pcc)
    @foreach($cities as $city)
    {{--@if($city->id==$pcc->cities)--}}
    @if(in_array($city->id,json_decode($pcc->cities)))

    <a href="{{ url("homesearch/{$typename}/{$course_slug}/{$city->name}") }}" class="pop">


      {{$coursename?? null}} coaching classes in  {{$city->name.' '.'|'.' '}}  </a>
...
@endif
<script>
    var assetPath = "{{ asset('img/empty-tree.svg') }}";
</script>
